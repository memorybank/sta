...
{
    //[CreateAssetMenu(fileName = "GeneralConfig", menuName = "FrostweepGames/GoogleCloud/StreamingSpeechRecognition/GeneralConfig", order = 3)]
    public class GeneralConfig : ScriptableObject
    {
        ...
        {
            string path = "StreamingSpeechRecognition/GeneralConfig";
            var config = Resources.Load<GeneralConfig>(path);

            if(config == null)
            {
                Debug.LogError($"Google Cloud Streaming Speech Recognition General Config not found in {path} Resources folder. Will use default.");

                config = (GeneralConfig)CreateInstance("GeneralConfig");

#if UNITY_EDITOR
                string pathToFolder = "Assets/Frostweepgames/StreamingSpeechRecognition/Resources/StreamingSpeechRecognition";
