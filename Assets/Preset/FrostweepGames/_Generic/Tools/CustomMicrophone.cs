...
namespace FrostweepGames.Plugins.Native
{
	public sealed class CustomMicrophone
	{
		...
		public static bool GetRawData(ref float[] output, float[] AudioBuffer)
        {
			// Debug.Log(String.Format("output {0} {1}", output.Length, AudioBuffer.buffer.Length));
			lock (AudioBuffer)
            {
				for (var i = 0; i < Math.Min(output.Length, AudioBuffer.Length); i++)
				{
